...
3. `npm run outline:pull -- --file docs/<path>.md` — тягне поточний текст
   із Outline назад у файл: відрізає авто-згенеровані блоки (бейджі,
   зміст, «Пов'язані документи», футер — вони перегенеруються при наступній
   публікації), качає нові зображення в `docs/public/outline-imports/<slug>/`
   і переписує посилання на локальні, конвертує Outline-URL (`/doc/…`,
   `/collection/…`) назад у канонічні git-шляхи.
4. Редактор перевіряє `git diff`, знімає `outline_locked`, комітить.
