...
| B-17 | P1 | done | wiki-eng | Outline: інлайн-крос-лінки биті. Новий [links.mjs](../scripts/outline/links.mjs) резолвить canonical-path → живий Outline doc/collection URL (кеш, read-only проти API) і переписує абсолютні, відносні (`./x.md`), секційні (`/x/y/`) та `[[wiki]]`-лінки, зберігаючи `#anchor`. Підсекційні `index.md` тепер публікуються (унікальні title), тому секційні лінки теж резолвляться; топ-рівневі index (title == назва колекції) лишаються неопублікованими, лінки на них ведуть на URL колекції. Перевірено на живому Outline (`npm run outline:reconcile`, 107/107 без помилок) | [links.mjs](../scripts/outline/links.mjs), [cleaners.mjs](../scripts/outline/cleaners.mjs) (`isTopLevelIndexPath`), [_publishOne.mjs](../scripts/outline/_publishOne.mjs) |
...
| B-38 | P2 | done | wiki-eng | Outline branding: Outline не має штатного механізму кастомного CSS. Піднято `outline-brand-proxy` (systemd, `:3100`) — інжектить same-origin `<link>` перед `</head>`, задовольняє CSP (`style-src 'self'`); Caddy для `wiki.secretshop.ua` тепер проксує на `:3100` замість `:3000` напряму. Юніт `enabled` (переживає ребут). Ризик: цілиться в HTML-shell/DOM Outline, апдейт Outline може тихо зламати стилі — CSS навмисно написаний на структурних селекторах, без хешованих `sc-*` класів | [ops/outline/branding/](outline/branding/README.md) |
| B-39 | P1 | done | wiki-eng | Outline → git зворотний потік (ручний): `outline_locked: true` (frontmatter) блокує sync/reconcile/publish від перезапису документа, поки редактор доопрацьовує його напряму в Outline (скріншоти, embed). `npm run outline:pull -- --file <path>` тягне поточний Outline-текст назад: відрізає авто-блоки (маркери `<!-- outline:name:start/end -->` в `enhanceForOutline`), качає зображення в `docs/public/outline-imports/<slug>/`, конвертує Outline `/doc/`, `/collection/` URL назад у канонічні git-шляхи (`links.mjs:rewriteOutlineLinksToCanonical`), повертає H1, збережений frontmatter. Перевірено live round-trip на реальному документі. Обмеження: не round-trip у зворотний бік (git-локальні картинки не резолвляться всередині Outline після повторної публікації); документи без маркерів (опубліковані до цієї фічі) вимагають одного `sync`/`reconcile`, перш ніж `pull` спрацює | [pull.mjs](../scripts/outline/pull.mjs), [links.mjs](../scripts/outline/links.mjs), [cleaners.mjs](../scripts/outline/cleaners.mjs) (`wrapMarker`, `readFrontmatterOutlineLocked`), `skills/company-wiki/references/frontmatter-schema.md#outline_locked` |
| B-40 | P1 | blocked | founders | Outline → git зворотний потік (автоматичний): `.github/workflows/outline-pull.yml`, cron кожні 15 хв — для кожного `outline_locked`-документа чистий checkout → `pull.mjs --file` → якщо є diff → окрема гілка `outline-pull/<slug>` → PR (ніколи не пушить у `main` напряму, завжди ручне рев'ю). Обрано GH Actions polling замість Outline-webhook: не додає нового публічного ендпоінта на сервері (на відміну від вже проблемного патерну з B-35), не потребує нового PAT/signing-secret на сервері (лише один GH secret), цикл git↔Outline неможливий — publish і так пропускає `outline_locked`-доки. **Заблоковано на ручному one-time сетапі, недоступному з коду**: (1) repo secret `OUTLINE_API_TOKEN`, (2) Settings → Actions → General → увімкнути «Allow GitHub Actions to create and approve pull requests». Публічний API-шлях (`https://wiki.secretshop.ua/api/…` без Authelia-гейту) перевірено живим запитом — воркфлоу технічно спрацює одразу після сетапу | [.github/workflows/outline-pull.yml](../.github/workflows/outline-pull.yml), [pull-locked.mjs](../scripts/outline/pull-locked.mjs) |
...
- 2026-07-17 — **B-17 done**: crosslink-резолвер (`links.mjs`) + публікація підсекційних `index.md`, перевірено live-reconcile 107/107. **B-38 done**: Outline CSS-брендинг через `outline-brand-proxy` (systemd, `:3100`), Caddy перемкнено з `:3000`; деталі й rollback у [ops/outline/branding/README.md](outline/branding/README.md).
