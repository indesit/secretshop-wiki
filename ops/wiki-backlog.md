...
| B-16 | P1 | in-progress | wiki-eng | Outline: усунути дублі в меню. **Код-фікс done** — `index.md` більше не публікується як документ ([_publishOne.mjs](../scripts/outline/_publishOne.mjs), [sync.mjs](../scripts/outline/sync.mjs)); [cleanup.mjs](../scripts/outline/cleanup.mjs) тепер ловить **усі** index-доки за назвами index.md з репо (і колекційні, і підрозділні — «Технічні несправності» тощо), + застарілі рос. колекції; [reconcile.mjs](../scripts/outline/reconcile.mjs) — повне перенаповнення. **Лишилось:** запустити cleanup+reconcile на живому Outline з токеном | `scripts/outline/*` |
